// Every country recognised by ISO 3166-1, each with its ISO 4217 currency
// code — the reference data behind the gym registration country picker
// (CountryPicker.jsx) and gym-currency money formatting (helpers.js's
// formatMoney). `code` is the country's alpha-2 code, ALSO used as the
// region subtag for Intl.NumberFormat (e.g. "en-KE") so amounts render with
// the currency's real-world symbol/grouping instead of a generic fallback —
// see the note on formatMoney in helpers.js for why that matters.
//
// One country, one currency here, even for the handful that legally allow
// more than one (e.g. Cuba) or that share a currency across a bloc (the
// Eurozone, the CFA franc zones, the East Caribbean dollar) — this is a gym
// management app's pricing currency, not a treasury system, so "the one
// everyone actually prices things in" is the right level of detail.
...
/** Look up a country by its alpha-2 code (gyms.country_code). */
export function countryByCode(code) {
  return COUNTRIES.find((c) => c.code === code) || null;
}

/** Case-insensitive substring match on name — the autocomplete's filter. */
